// resources/js/pages/Owner/Grooming/Create.tsx
...
export default function GroomingCreate() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setSaving(true);
        const res = await groomingService.createSession(Number(petId), { ...form, service_type: form.service_type as any || undefined });
        setSaving(false);
        if (res.success) {
    ...
    return (
        <OwnerLayout>
            <Head title="Log Grooming Session — Happy Tails" />
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <textarea rows={3} value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="Any notes…" className={`${f('notes')} resize-none`} />
                            </div>
                        </div>
