// resources/js/pages/Owner/Vaccinations/Index.tsx
import { Head, Link, usePage } from '@inertiajs/react';
import { useEffect, useState } from 'react';
import { Syringe, Plus, ChevronLeft, Edit2, Trash2, Loader2, AlertCircle } from 'lucide-react';
import Swal from 'sweetalert2';
import OwnerLayout from '@/layouts/owner-layout';
import { vaccinationService } from '@/services/happy-tails';
import type { Vaccination } from '@/services/happy-tails/types';
...
export default function VaccinationsIndex() {
    ...
    useEffect(() => {
        vaccinationService.getVaccinations(Number(petId)).then(r => { if (r.data) setVaccs(r.data); setLoading(false); });
    }, [petId]);
    ...
    const handleDelete = async (id: number) => {
        ...
        if (r.isConfirmed) {
            await vaccinationService.deleteVaccination(Number(petId), id);
    ...
    return (
        <OwnerLayout>
            <Head title="Vaccinations — Happy Tails" />
            <div className="min-h-screen bg-slate-50">
                <div className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white">
                    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 flex items-center justify-between gap-4 flex-wrap">
                        <div>
                            <Link href={route('owner.pets.show', petId)} className="inline-flex items-center gap-1 text-blue-200 hover:text-white text-sm font-semibold mb-3"><ChevronLeft className="w-4 h-4" /> Back</Link>
                            <h1 className="text-2xl font-black flex items-center gap-2"><Syringe className="w-6 h-6" /> Vaccinations</h1>
                        </div>
                        <Link href={route('owner.pets.vaccinations.create', petId)} className="flex items-center gap-2 px-5 py-2.5 bg-white text-blue-600 rounded-xl font-black text-sm hover:shadow-xl hover:scale-105 transition-all">
                ...
                <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
                    ...
                    : vaccs.length === 0 ? (
                        <div className="text-center py-14 bg-white rounded-2xl border border-gray-100 shadow-sm">
                            ...
                            <p className="text-gray-500 font-bold">No vaccinations yet</p>
                            <Link href={route('owner.pets.vaccinations.create', petId)} className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-500 text-white rounded-xl text-sm font-bold mt-4 hover:bg-blue-600 transition-colors"><Plus className="w-3.5 h-3.5" /> Add Vaccination</Link>
                        </div>
                    ) : (
                            ...
                            {vaccs.map(v => (
                                <div key={v.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
                                    ...
                                    <div className="flex gap-2 shrink-0">
                                        <Link href={route('owner.pets.vaccinations.edit', { petId, id: v.id })} className="p-2 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-500 transition-colors"><Edit2 className="w-4 h-4" /></Link>
