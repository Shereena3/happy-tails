// resources/js/pages/Owner/Medicines/Create.tsx
...
export default function MedicineCreate() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setSaving(true);
        const res = await medicineService.createMedicine(Number(petId), form);
        setSaving(false);
        if (res.success) {
    ...
    return (
        <OwnerLayout>
            <Head title="Add Medicine — Happy Tails" />
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <textarea rows={3} value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="Additional notes…" className={`${f('notes')} resize-none`} />
                            </div>
                        </div>
