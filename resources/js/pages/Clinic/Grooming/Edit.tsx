// resources/js/pages/Clinic/Grooming/Edit.tsx
...
export default function ClinicGroomingEdit() {
    ...
    useEffect(() => {
        fetch(`/api/clinic/pets/${petId}/grooming/${sessionId}`, { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' }, credentials: 'same-origin' })
            .then(r => r.json()).then(j => {
                if (j.success && j.data) { const s = j.data; setForm({ date: s.date, service_type: s.service_type ?? '', notes: s.notes ?? '' }); }
                setLoading(false);
            });
    }, [petId, sessionId]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setSaving(true);
        const res = await fetch(`/api/clinic/pets/${petId}/grooming/${sessionId}`, {
            method: 'PUT',
    ...
    if (loading) return <ClinicLayout><div className="flex items-center justify-center min-h-screen"><Loader2 className="w-8 h-8 animate-spin text-pink-300" /></div></ClinicLayout>;

    return (
        <ClinicLayout>
            <Head title="Edit Grooming — Clinic" />
