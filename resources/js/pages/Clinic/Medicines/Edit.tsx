// resources/js/pages/Clinic/Medicines/Edit.tsx
...
export default function ClinicMedicineEdit() {
    ...
    useEffect(() => {
        fetch(`/api/clinic/pets/${petId}/medicines/${medicineId}`, { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' }, credentials: 'same-origin' })
            .then(r => r.json()).then(j => {
                if (j.success && j.data) { const m = j.data; setForm({ medicine_name: m.medicine_name, dosage: m.dosage ?? '', start_date: m.start_date ?? '', end_date: m.end_date ?? '', notes: m.notes ?? '' }); }
                setLoading(false);
            });
    }, [petId, medicineId]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setSaving(true);
        const res = await fetch(`/api/clinic/pets/${petId}/medicines/${medicineId}`, {
            method: 'PUT',
    ...
    if (loading) return <ClinicLayout><div className="flex items-center justify-center min-h-screen"><Loader2 className="w-8 h-8 animate-spin text-green-300" /></div></ClinicLayout>;

    return (
        <ClinicLayout>
            <Head title="Edit Medicine — Clinic" />
